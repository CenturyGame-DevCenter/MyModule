...
namespace CenturyGame.Framework.Network
{
    public class TcpClient : NetworkClient
    {
        ...
        public void CloseSocket()
        {
            ...
            {
                Trace.Instance.warn("init socket throw exception. exception = {0}", e.Message);
            }
        }
        ...
        private void HandleReceive(IAsyncResult result)
        {
            try
            {
                ...
                {
                    CloseSocket();
                    PostOptionMsgEvent?.Invoke(new DisconnectMsg(DisconnectReason.Passive));
                }
            }
            catch (Exception e)
            {
                Trace.Instance.warn("recevie message throw exception. exception = {0}", e.Message);
                PostOptionMsgEvent?.Invoke(new DisconnectMsg(DisconnectReason.Exception));
            }
        }
        ...
        public override void Disconnect()
        {
            ...
            {
                CloseSocket();
                PostOptionMsgEvent?.Invoke(new DisconnectMsg(DisconnectReason.Active));
            }
        }
